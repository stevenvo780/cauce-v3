...
**Enrutado:** sin react-router — `src/navigation.ts` (`matchRoute` + `ROUTE_TABLE`/`ROUTE_ALIASES`) y `src/App.tsx`. Vistas: Portada, live (flota + cajón de agente con directiva/perfil/ficheros), accounts, messages, queues, observability, config y terminal (xterm.js vía `src/features/terminal/pty-session.ts`, WS same-origin proxificado al relay).
