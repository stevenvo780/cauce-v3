...
Hoy se movieron decenas de ficheros (censo, particiones, extracciones). Repite tu verificación de ronda 3 sobre: `README.md`, `docs/arquitectura.md` (¡sus tablas citan rutas que pueden haber cambiado: pty-session partido, repository/ modular!), los README de componentes, `plan-reestructura/**`, `ordenes/**`, `_legado/README.md`. Corrige lo obvio, reporta lo dudoso en `docs/bitacora/reportes/minimax-enlaces-r5.md`.
...
`grep -rn "TODO\|FIXME\|HACK\|XXX" --include="*.ts" --include="*.tsx" --include="*.py" --include="*.sh"` sobre el árbol vivo (fuera de `_legado/`, tests incluidos): tabla en `docs/bitacora/reportes/minimax-todos.md` con fichero:línea → texto → tu clasificación (deuda real / obsoleto / ruido). No edites código.
