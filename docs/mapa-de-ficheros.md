...
- C. apps/console/ + scripts/ + vitest.config.ts (286) — Gemini + Codex
- D. ops/ + deploy/ (108) — Codex + Claude+FASE 3 + DUEÑO

Hallazgos del ensamblado:
- **1** entrada del subagente apuntaba a un path que no existe en `git ls-files` — descartada (probable alucinación: `apps/console/src/features/terminal/SessionStage.test.tsx`).
- **1** entrada duplicada dentro de la misma sección — descartada.
- Co-localización: muchos tests viven junto a su código bajo `src/` (p. ej. `services/gateway/src/*.test.ts`, `apps/console/src/**/*.test.tsx`) y están listados en su grupo (A–D). Los tests independientes bajo directorios `test/` van en la sección E.

Notas metodológicas:
- Los subagentes LEYERON cada fichero (no infirieron del nombre). Las descripciones son verificables abriendo el fichero.
- Cuando el nombre del fichero sugería algo distinto de lo que hace, marcaron ⚠ (8 casos: 2 en `apps/console`, 5 wrappers delgados en `ops/scripts/dlq-*` y `telegram-manual-replay/replay-inspect`, 1 shim en `ops/guardias/cauce-huerfanas.sh`). Los 5 wrappers coinciden con la "Familia DLQ manual" marcada como dudosa en `PENDIENTES-DEL-DUEÑO.md` §(2)(b).
...
## C. apps/console/ + scripts/ + vitest.config.ts

### apps/console/src/ (279x)
- apps/console/src/App.invariantes.test.tsx — invariantes estructurales de la tabla de rutas: ids duplicados, componentes sin definir, alias sin destino real — Gemini (consola)
- apps/console/src/App.test.tsx — pruebas de integración del router: reescritura de alias, fallback 404, deep-link a /fleet/:tenant/:alias — Gemini (consola)
- apps/console/src/App.tsx — router principal con reescritura de alias (licenses→accounts, activity→live, etc.) — Gemini (consola)
- apps/console/src/main.tsx — bootstrap ReactDOM + arranque condicional de MSW según VITE_USE_MOCKS — Gemini (consola)
- apps/console/src/api/audit-client.test.ts — pruebas de `listAudit()` contra el handler mock — Gemini (consola)
- apps/console/src/api/client.test.ts — pruebas de fetch con CSRF, cookie credentials y borrado de campos hostiles — Gemini (consola)
- apps/console/src/api/client.timeout.test.ts — suite de timeouts con fetch que nunca responde — Gemini (consola)
- apps/console/src/api/client.ts — clase `CauceApi` que envuelve fetch con timeout 30s, headers `X-Cauce-Console` y propagación de CSRF — Gemini (consola)
- apps/console/src/api/client/agent-client.ts — métodos agent-* (perfil, directiva, documentos) sobre `CauceApi` — Gemini (consola)
- apps/console/src/api/client/core.ts — núcleo HTTP: timeout AbortController, errores tipados `PublishIntentReconciliation/Expired/RateLimited` — Gemini (consola)
- apps/console/src/api/client/messaging-client.ts — publishMessage, preparePublishIntent, confirmPublishIntent con tipos exactos — Gemini (consola)
- apps/console/src/api/client/system-client.ts — auth/session, access, topology, config, observability, audit — Gemini (consola)
- apps/console/src/api/context.tsx — provider `ApiProvider` y hook `useApi()` — Gemini (consola)
- apps/console/src/api/types.ts — re-exports de los schemas por dominio — Gemini (consola)
- apps/console/src/api/types/activity.ts — FleetActivitySnapshot, FleetActivityAgent, FleetActivityItem, FleetDelegationEdge — Gemini (consola)
- apps/console/src/api/types/agent-directives.ts — tipos para perfil, directiva (capas 1-3), historial de rol y documentos — Gemini (consola)
- apps/console/src/api/types/auth.ts — ConsolePermission, LoginMode, ConsoleAuthState, ConsoleAccess — Gemini (consola)
- apps/console/src/api/types/chains.ts — AgentChainEdge, AgentChainNode, AgentChainSnapshot, AgentChainCounters — Gemini (consola)
- apps/console/src/api/types/config.ts — ConfigResource, RegistryConfigResource, ConfigAction, ConfigMutation — Gemini (consola)
- apps/console/src/api/types/deliveries.ts — DeliveryState, JobLane, TimelineEvent, DeliveryView, MessageView, MessagePage — Gemini (consola)
- apps/console/src/api/types/dlq.ts — DlqTarget, DlqDisposition, DlqItem, DlqPage, ResolveDlqWithoutReplayResult — Gemini (consola)
- apps/console/src/api/types/quotas.ts — QuotaSeverity, QuotaWindow, QuotaGroup, QuotaProviderReport, QuotaCollector, QuotaUnboundGroup, QuotaPausedAccount, QuotaThresholds, QuotaSnapshot — Gemini (consola)
- apps/console/src/api/types/system.ts — CapabilityState, PresenceLease, SystemStatus, TerminalCapability, ObservabilitySnapshot — Gemini (consola)
- apps/console/src/api/types/topology.ts — MemberOffReason, RoomMember, TenantNode, AclEdge, TopologySnapshot — Gemini (consola)
- apps/console/src/api/use-resource.fallo-visible.test.tsx — pruebas de fallo de red: timeout, 5xx, mensaje legible — Gemini (consola)
- apps/console/src/api/use-resource.test.tsx — smoke del hook `useResource` con polling manual — Gemini (consola)
- apps/console/src/api/use-resource.ts — hook genérico de lectura de recursos con `key` por scope y manejo de cache — Gemini (consola)
- apps/console/src/components/Tooltip.test.tsx — accesibilidad y disparo del globo en hover/focus/Esc — Gemini (consola)
- apps/console/src/components/Tooltip.tsx — ⚠ el nombre miente: es la primitiva `Tooltip`/`FloatingTooltip` con `createPortal` y `aria-describedby` — Gemini (consola)
- apps/console/src/components/ui.tsx — primitivas reusables: `Badge`, `Panel`, `PageHeader`, `Metric`, `LoadingState`, `ErrorState`, `ViewTabs`, `Unknown` — Gemini (consola)
- apps/console/src/components/view-tabs-legibilidad.test.ts — `ViewTabs` legible en tema claro (contraste ≥4.5:1) — Gemini (consola)
- apps/console/src/contraste-cascada.test.ts — pruebas unitarias de contraste heredado en hojas globales — Gemini (consola)
- apps/console/src/features/accounts/AccountRoutingDetail.tsx — ficha desplegable con plan, pagador, agentes asignados, techo de ruteo — Gemini (consola)
- apps/console/src/features/accounts/AccountsInventory.tsx — tabla maestra con inventario/edición/altabaja de cuentas de IA y barra de pool — Gemini (consola)
- apps/console/src/features/accounts/AccountsPage.test.tsx — render con fixtures: 38 filas, edición inline, validaciones de IDs — Gemini (consola)
- apps/console/src/features/accounts/AccountsPage.tsx — orquesta AccountsInventory + ConsumptionSection + AssignmentMatrix — Gemini (consola)
- apps/console/src/features/accounts/AssignmentMatrix.test.tsx — render de la matriz agente×cuenta — Gemini (consola)
- apps/console/src/features/accounts/AssignmentMatrix.tsx — matriz interactiva de asignación cuenta×agente — Gemini (consola)
- apps/console/src/features/accounts/ConsumptionSection.test.tsx — render del panel de consumo — Gemini (consola)
- apps/console/src/features/accounts/ConsumptionSection.tsx — panel de consumo por cuenta y ventana de cuota — Gemini (consola)
- apps/console/src/features/accounts/MutationBar.tsx — barra inferior con previsualizar/aplicar y mensaje de error — Gemini (consola)
- apps/console/src/features/accounts/Sparkline.tsx — sparkline SVG minimalista con datos submuestreados — Gemini (consola)
- apps/console/src/features/accounts/licenses-calculation.test.ts — cálculo de licencias disponibles/expiradas — Gemini (consola)
- apps/console/src/features/accounts/licenses.test.ts — pruebas del módulo de licencias — Gemini (consola)
- apps/console/src/features/accounts/licenses.ts — cálculo de licencias: vigentes/expiradas/pool — Gemini (consola)
- apps/console/src/features/accounts/quotas.test.ts — agregación de grupos por familia y peor ventana — Gemini (consola)
- apps/console/src/features/accounts/quotas.ts — agregación de cuotas por familia (antigravity) y selección de peor ventana — Gemini (consola)
- apps/console/src/features/accounts/registry.test.ts — lectura y validación de la lista de agentes del registro — Gemini (consola)
- apps/console/src/features/accounts/registry.ts — lectura normalizada del registro de agentes (membership/flag/pool) — Gemini (consola)
- apps/console/src/features/accounts/use-registry-mutation.ts — hook que aplica cambios sobre el registro de agentes vía config — Gemini (consola)
- apps/console/src/features/audit/AuditPanel.test.tsx — paginación keyset con filtros y traducciones JSON — Gemini (consola)
- apps/console/src/features/audit/AuditPanel.tsx — paginador keyset del audit log con buscador — Gemini (consola)
- apps/console/src/features/audit/audit-summary.test.ts — traduce JSON allowlist del audit a frases humanas — Gemini (consola)
- apps/console/src/features/audit/audit-summary.ts — formateador de resúmenes estructurados del audit log — Gemini (consola)
- apps/console/src/features/auth/AuthGate.test.tsx — pruebas de la puerta de sesión (loading/sin sesión/sesión/permiso) — Gemini (consola)
- apps/console/src/features/auth/AuthGate.tsx — puerta de sesión con login por contraseña o redirect OIDC y badge en la topbar — Gemini (consola)
- apps/console/src/features/auth/auth-session.ts — hook `useAuthGate` que consulta `/v3/auth/session` y revalida cada 60s — Gemini (consola)
- apps/console/src/features/config/AltaDeEspacios.tsx — exporta el alta de espacios (AltaRapida | SpaceWizard) — Gemini (consola)
- apps/console/src/features/config/AltaRapida.tsx — formulario de alta de un solo recurso (membership/acl_edge/room/agent) — Gemini (consola)
- apps/console/src/features/config/ArnesesPanel.tsx — panel «qué lee cada arnés» con 4 arneses (claude/codex/openclaw/hermes) — Gemini (consola)
- apps/console/src/features/config/CollectionTable.tsx — tabla interactiva con interruptores para colecciones de configuración — Gemini (consola)
- apps/console/src/features/config/ConfigPage.actions.test.tsx — pruebas de las acciones de tabla (toggles, cambios de rol, confirm) — Gemini (consola)
- apps/console/src/features/config/ConfigPage.inertes.test.tsx — pruebas de la pestaña «Agentes y cuentas» con arneses inertes — Gemini (consola)
- apps/console/src/features/config/ConfigPage.tables.test.tsx — render de tablas con/sin campos inertes — Gemini (consola)
- apps/console/src/features/config/ConfigPage.test.tsx — tests integración de ConfigPage (permisos, fallback 403) — Gemini (consola)
- apps/console/src/features/config/ConfigPage.tsx — página «Ajustes y altas» con 7 pestañas (config, espacios, permisos, arneses, roles, etc.) — Gemini (consola)
- apps/console/src/features/config/Interruptor.tsx — switch accesible con tooltip/cabecera/diálogo de confirmación — Gemini (consola)
- apps/console/src/features/config/RolesPanel.tsx — panel de políticas de rol (route/read/control/notify) — Gemini (consola)
- apps/console/src/features/config/SpaceWizard.test.tsx — pruebas del wizard de espacios — Gemini (consola)
- apps/console/src/features/config/SpaceWizard.tsx — wizard guiado tenant→room→membership→harness — Gemini (consola)
- apps/console/src/features/config/alta-rapida.test.ts — validaciones del alta rápida con regex — Gemini (consola)
- apps/console/src/features/config/alta-rapida.ts — validación de formularios contra regex de protocolo — Gemini (consola)
- apps/console/src/features/config/areas.test.ts — pruebas del mapa de áreas y su agrupador — Gemini (consola)
- apps/console/src/features/config/areas.ts — mapa de las 7 áreas de configuración y agrupador — Gemini (consola)
- apps/console/src/features/config/arneses.test.ts — cobertura del catálogo de arneses y frases — Gemini (consola)
- apps/console/src/features/config/arneses.ts — catálogo de 4 arneses con su ruta de directiva y dónde se toca — Gemini (consola)
- apps/console/src/features/config/campos-inertes.test.ts — pruebas del catálogo de campos sin efecto — Gemini (consola)
- apps/console/src/features/config/campos-inertes.ts — catálogo de campos sin lector runtime con su cita al código — Gemini (consola)
- apps/console/src/features/config/collection-table.test.ts — pruebas de CollectionTable — Gemini (consola)
- apps/console/src/features/config/collection-table.ts — tabla de colecciones con interruptores y diálogos — Gemini (consola)
- apps/console/src/features/config/collections.test.ts — derivación de colecciones del snapshot — Gemini (consola)
- apps/console/src/features/config/collections.ts — derivación de las colecciones de `/v3/console/config` — Gemini (consola)
- apps/console/src/features/config/config-change.ts — descripción legible de errores de cambio y helper `textoRecarga` — Gemini (consola)
- apps/console/src/features/config/config-receipt.test.ts — validador exacto de la respuesta a `config changes` — Gemini (consola)
- apps/console/src/features/config/config-receipt.ts — guarda de tipo estructural para el recibo de cambios — Gemini (consola)
- apps/console/src/features/config/config-css-toggles.test.ts — pruebas estructurales de los interruptores de config (espec. CSS vs `.metrics-grid.three`) — Gemini (consola)
- apps/console/src/features/config/config-css.test.ts — pruebas de legibilidad CSS de /config (tope, tipos de letra, contraste) — Gemini (consola)
- apps/console/src/features/config/fecha-relativa.test.ts — pruebas de fechas relativas en distintos formatos — Gemini (consola)
- apps/console/src/features/config/fecha-relativa.ts — formateador «hace X / en X» con corte 60 días — Gemini (consola)
- apps/console/src/features/config/Interruptores.test.tsx — pruebas de la lógica de interruptores con control negativo — Gemini (consola)
- apps/console/src/features/config/interruptores.test.ts — pruebas del módulo puro `interruptores.ts` (sin montar ConfigPage) — Gemini (consola)
- apps/console/src/features/config/interruptores.ts — lógica de interruptores con confirmación para `allow_control` — Gemini (consola)
- apps/console/src/features/config/roles.test.ts — cobertura de la lógica de RolesPanel — Gemini (consola)
- apps/console/src/features/config/roles.ts — extracción y render de políticas de rol — Gemini (consola)
- apps/console/src/features/config/use-interruptores.ts — hook que combina snapshot + optimistic + rollback — Gemini (consola)
- apps/console/src/features/fleet/FleetAgentDetailPage.test.tsx — render del detalle de un bot con TUI/PTY — Gemini (consola)
- apps/console/src/features/fleet/FleetAgentDetailPage.tsx — `/fleet/:tenant/:alias`: delega en OperatorWorkspace acotado al bot elegido — Gemini (consola)
- apps/console/src/features/help/HelpPage.tsx — página de ayuda con mapa de vistas, estados y atajos de teclado — Gemini (consola)
- apps/console/src/features/landing/HarnessStrip.tsx — tira plegada con los arneses declarados por el servidor — Gemini (consola)
- apps/console/src/features/landing/JobsRetiredNotice.tsx — aviso para la ruta retirada `/jobs` que señala adónde ir — Gemini (consola)
- apps/console/src/features/landing/LandingPage.permisos.test.tsx — la portada NO duplica el menú lateral — Gemini (consola)
- apps/console/src/features/landing/LandingPage.test.tsx — render de la portada con todas las alertas — Gemini (consola)
- apps/console/src/features/landing/LandingPage.tsx — portada con métricas, alertas agrupadas por vista y tira de arneses — Gemini (consola)
- apps/console/src/features/landing/landing.test.ts — pruebas unitarias del resumider de la portada — Gemini (consola)
- apps/console/src/features/landing/landing.ts — lógica pura del resumen de portada (alertas por destino) — Gemini (consola)
- apps/console/src/features/live/AgentAvatar.tsx — SVG animado por estado (down/blocked/working/idle) — Gemini (consola)
- apps/console/src/features/live/AgentDrawer.tsx — cajón lateral con 7 pestañas (ahora/conexión/entregas/cadena/directiva/perfil/ficheros) — Gemini (consola)
- apps/console/src/features/live/AgentTooltipCard.tsx — tarjeta de tooltip del hipergrafo (4 líneas + pie) — Gemini (consola)
- apps/console/src/features/live/ChainPanel.test.tsx — render de la cadena de delegación con aristas censuradas — Gemini (consola)
- apps/console/src/features/live/ChainPanel.tsx — panel de cadena de delegación con extremos opacos — Gemini (consola)
- apps/console/src/features/live/DirectivaModal.test.tsx — pruebas del modal de directiva (publicado/medido/no-mirado) — Gemini (consola)
- apps/console/src/features/live/DirectivaModal.tsx — modal con las tres capas (rol, manual, memoria) y sus avisos — Gemini (consola)
- apps/console/src/features/live/DirectivaTab.test.tsx — render de la pestaña de directiva — Gemini (consola)
- apps/console/src/features/live/DirectivaTab.tsx — pestaña resumen de directiva con modal a pantalla completa — Gemini (consola)
- apps/console/src/features/live/FicherosTab.test.tsx — render de editor/visor de ficheros con CAS — Gemini (consola)
- apps/console/src/features/live/FicherosTab.tsx — editor/visor de ficheros (CLAUDE.md, AGENTS.md, etc.) con guardado durable — Gemini (consola)
- apps/console/src/features/live/FleetActivityTable.test.tsx — render de la tabla con orden por urgencia y filas truncadas — Gemini (consola)
- apps/console/src/features/live/FleetActivityTable.tsx — tabla de agentes en vuelo con búsqueda por alias y orden por urgencia — Gemini (consola)
- apps/console/src/features/live/FleetVerdict.tsx — banda superior «veredicto de la flota» con culprits clicables — Gemini (consola)
- apps/console/src/features/live/HistorialRol.test.tsx — render del historial de cambios de rol con restauración a borrador — Gemini (consola)
- apps/console/src/features/live/HistorialRol.tsx — diario del rol declarado con puente seguro al editor canónico — Gemini (consola)
- apps/console/src/features/live/LiveFleetLegend.tsx — desplegable de Señales/Permisos/Salas/Estados — Gemini (consola)
- apps/console/src/features/live/LiveFleetPage.filters.test.tsx — pruebas de los filtros (búsqueda/cliente/estado) — Gemini (consola)
- apps/console/src/features/live/LiveFleetPage.sin-salida.test.tsx — render sin ningún dato en vuelo — Gemini (consola)
- apps/console/src/features/live/LiveFleetPage.test.tsx — render completo con el catálogo de mock fixtures — Gemini (consola)
- apps/console/src/features/live/LiveFleetPage.tsx — orquesta `LiveFleetToolbar` + `FleetVerdict` + `Tally` + hipergrafo + tabla + cajón — Gemini (consola)
- apps/console/src/features/live/LiveFleetTally.tsx — cinta de conteo por estado (caído/trabado/…) — Gemini (consola)
- apps/console/src/features/live/LiveFleetToolbar.tsx — barra superior con polling y filtros — Gemini (consola)
- apps/console/src/features/live/LiveHypergraph.tsx — SVG del mapa de flota con nodos, salas, flechas ACL y capa de personas — Gemini (consola)
- apps/console/src/features/live/PerfilTab.test.tsx — render del editor de perfil con guardado governed y ACK exacto — Gemini (consola)
- apps/console/src/features/live/PerfilTab.tsx — editor y preview del perfil canónico del agente (7 campos) — Gemini (consola)
- apps/console/src/features/live/RoleBriefTab.test.tsx — pruebas de la proyección legacy del rol — Gemini (consola)
- apps/console/src/features/live/RoleBriefTab.tsx — proyección de solo-lectura del `role_brief` con puente al editor canónico — Gemini (consola)
- apps/console/src/features/live/activity.test.ts — pruebas de la lógica de actividad (formatos, orden, deduplicación) — Gemini (consola)
- apps/console/src/features/live/activity.ts — etiqueta de work states, deduplicación de señales, formato de ACK — Gemini (consola)
- apps/console/src/features/live/agent-state-derivation.test.ts — derivación de `LiveState` y construcción de la vista — Gemini (consola)
- apps/console/src/features/live/agent-state-helpers.ts — `LiveState` + `LiveAgentView` + aristas agregadas — Gemini (consola)
- apps/console/src/features/live/agent-state.test.ts — pruebas del veredicto, los 7 estados y el pulso de entrega — Gemini (consola)
- apps/console/src/features/live/agent-state.ts — `liveState` (down/blocked/…) y `fleetVerdict` con 3 cubos (problema/ocupado/libre) — Gemini (consola)
- apps/console/src/features/live/capas-pendientes.ts — hueco rotulado de las dos capas que la consola no edita todavía — Gemini (consola)
- apps/console/src/features/live/deriva.test.ts — pruebas de `derivaDelRegistro` (sin registro, sin sala, ambos) — Gemini (consola)
- apps/console/src/features/live/deriva.ts — diferencia simétrica entre `agents` y membresías — Gemini (consola)
- apps/console/src/features/live/directiva.test.ts — pruebas de `avisosDeCapas` con control negativo — Gemini (consola)
- apps/console/src/features/live/directiva.ts — detección de autonomía duplicada y manual abriendo con «Sos…» — Gemini (consola)
- apps/console/src/features/live/directiva-modal/AvisosDeSolapamiento.tsx — caja de avisos de solapamiento de capas — Gemini (consola)
- apps/console/src/features/live/directiva-modal/CapaCabecera.tsx — cabecera de cada capa (icono, número, fuente, ¿por qué?) — Gemini (consola)
- apps/console/src/features/live/directiva-modal/CapasPendientes.tsx — desplegable de huecos «herramientas y prompts» — Gemini (consola)
- apps/console/src/features/live/directiva-modal/ContenidoDeCapas.tsx — cuerpos de las tres capas (manual, memoria) con su medición — Gemini (consola)
- apps/console/src/features/live/estado-de-la-fila.test.tsx — pruebas de coherencia fila ↔ chip del veredicto de la flota — Gemini (consola)
- apps/console/src/features/live/ficheros-legibilidad.test.ts — pruebas de legibilidad CSS de los avisos de ficheros — Gemini (consola)
- apps/console/src/features/live/ficheros.test.ts — pruebas de los motivos de fallo por status HTTP — Gemini (consola)
- apps/console/src/features/live/ficheros.ts — reglas de edición/visualización según `editable`/`projected_fields` — Gemini (consola)
- apps/console/src/features/live/historial-rol.test.ts — pruebas de `entradasMasNuevasPrimero`, `resumirCambio`, `estadoDelDiario` — Gemini (consola)
- apps/console/src/features/live/historial-rol.ts — ordenamiento, resumen y distinción de los 3 desenlaces del diario — Gemini (consola)
- apps/console/src/features/live/live-hypergraph/FlowArrow.tsx — flecha SVG con animación SMIL para delegaciones en vuelo — Gemini (consola)
- apps/console/src/features/live/medicion-de-capa.test.ts — pruebas del discriminante `medido`/`files`/`memory` — Gemini (consola)
- apps/console/src/features/live/perfil-css.test.ts — pruebas estructurales del editor de perfil en móvil/escritorio — Gemini (consola)
- apps/console/src/features/live/perfil.test.ts — pruebas del editor (CAS, topes UTF-16 vs code-points, ACK exacto) — Gemini (consola)
- apps/console/src/features/live/perfil.ts — editor canónico de perfil: 7 campos, topes, ACK de runtime — Gemini (consola)
- apps/console/src/features/live/role-brief-runtime.test.ts — pruebas de la guarda `bloqueoPorRuntimeDesplegado` — Gemini (consola)
- apps/console/src/features/live/role-brief.ts — tope del rol y guarda UTF-16 vs code-points — Gemini (consola)
- apps/console/src/features/live/tira-de-pestanas.test.ts — pruebas del ancho de la tira de pestañas del cajón — Gemini (consola)
- apps/console/src/features/live/veredicto-vocabulario.test.ts — coherencia entre veredicto y chips de la cinta — Gemini (consola)
- apps/console/src/features/live/vocabulario-de-estados.test.ts — tabla de equivalencias work_state→LiveState — Gemini (consola)
- apps/console/src/features/messages/AgentRoster.tsx — lista lateral de agentes con sus pildoras de cola (en cola/curso/reintentos/muertas) — Gemini (consola)
- apps/console/src/features/messages/ConversationPane.tsx — panel de conversación con hilo, detalle completo, compositor anclado — Gemini (consola)
- apps/console/src/features/messages/MessageTimeline.test.tsx — render de la timeline de una entrega — Gemini (consola)
- apps/console/src/features/messages/MessageTimeline.tsx — timeline published→accepted→started→done/failed de una entrega — Gemini (consola)
- apps/console/src/features/messages/MessagesPage.test.tsx — render con mocks: compositor anclado, enlace profundo, fan-out — Gemini (consola)
- apps/console/src/features/messages/MessagesPage.tsx — `/messages` con roster lateral + panel + polling + composer anclado — Gemini (consola)
- apps/console/src/features/messages/composer-anclado.test.ts — pruebas CSS del anclaje del compositor en móvil y escritorio — Gemini (consola)
- apps/console/src/features/messages/desplazamiento.test.ts — pruebas de `irAlFinal`/`estaPegadoAlFinal` con `scrollTo` — Gemini (consola)
- apps/console/src/features/messages/desplazamiento.ts — utilidades de scroll: «ir al final» respetando lo que el operador leyó arriba — Gemini (consola)
- apps/console/src/features/messages/durable-publish.test.ts — pruebas del flujo durable con reconciliación de journal — Gemini (consola)
- apps/console/src/features/messages/durable-publish.ts — flujo de publicación durable con nonce + prepare + confirm + retry — Gemini (consola)
- apps/console/src/features/messages/hilo-legible.test.tsx — tres defectos medidos del hilo (selección fantasma, abre por el viejo, recorte sin etiqueta) — Gemini (consola)
- apps/console/src/features/messages/messages-css.test.ts — pruebas de las clases CSS usadas vs definidas — Gemini (consola)
- apps/console/src/features/messages/publish-receipt.test.ts — pruebas del validador de recibos durables — Gemini (consola)
- apps/console/src/features/messages/publish-receipt.ts — validador de tipo estructural del recibo durable — Gemini (consola)
- apps/console/src/features/messages/queue-health.test.ts — pruebas de `saludDeColaPorAgente` (UNKNOWN vs cero) — Gemini (consola)
- apps/console/src/features/messages/queue-health.ts — fusión de `/v3/console/activity` + `/v3/console/queues` por agente — Gemini (consola)
- apps/console/src/features/messages/roster.test.ts — pruebas del universo ampliado del roster (membresía/registro/mensajes) — Gemini (consola)
- apps/console/src/features/messages/roster.ts — construcción del roster de mensajería con 4 fuentes (topología/registro/mensajes/presencia) — Gemini (consola)
- apps/console/src/features/observability/ObservabilityPage.test.tsx — render con relays auditables y cruce contra audit — Gemini (consola)
- apps/console/src/features/observability/ObservabilityPage.tsx — página «Señales y auditoría» con 2 pestañas y relays al origen — Gemini (consola)
- apps/console/src/features/queues/DeliveryTable.test.tsx — render con replay/cancel y revisión tras 202 sin recibo — Gemini (consola)
- apps/console/src/features/queues/DeliveryTable.tsx — tabla de entregas con replay/cancel tras diálogo de confirmación — Gemini (consola)
- apps/console/src/features/queues/OperationalDlqPanel.test.tsx — paginación keyset del DLQ operativo y reconciliación — Gemini (consola)
- apps/console/src/features/queues/OperationalDlqPanel.tsx — panel del DLQ operativo con paginación, motivos y resolución sin replay — Gemini (consola)
- apps/console/src/features/queues/QueuesPage.test.tsx — pruebas de QueuesPage con 38 filas y filtros — Gemini (consola)
- apps/console/src/features/queues/QueuesPage.tsx — página «Colas y DLQ operativo» con búsqueda, link profundo y DLQ — Gemini (consola)
- apps/console/src/features/queues/colas-accionables.test.tsx — pruebas de los 3 defectos medidos del carrusel de colas (sin filtro, ámbar en done, reinyectar sin confirmar) — Gemini (consola)
- apps/console/src/features/queues/colas-puras.test.ts — pruebas de la lógica de filtro y de `leerUltimoError` — Gemini (consola)
- apps/console/src/features/queues/delivery-receipts.test.ts — pruebas del validador de recibos de replay/cancel — Gemini (consola)
- apps/console/src/features/queues/delivery-receipts.ts — validador de tipo para recibos de replay/cancel — Gemini (consola)
- apps/console/src/features/queues/filtro-de-colas.ts — filtro por grupo (revision/retry/pendientes) y texto libre — Gemini (consola)
- apps/console/src/features/queues/foco-de-entrega.test.ts — pruebas del deep-link `/queues?delivery=` — Gemini (consola)
- apps/console/src/features/queues/foco-de-entrega.ts — deep-link a una entrega específica, con manejo de ausente — Gemini (consola)
- apps/console/src/features/queues/ultimo-error.ts — clasificación «texto»/«sin-error»/«desconocido» por estado de entrega — Gemini (consola)
- apps/console/src/features/terminal/AckInspector.test.tsx — pruebas de la timeline y acciones de replay/cancel — Gemini (consola)
- apps/console/src/features/terminal/AckInspector.tsx — inspector de timeline de una entrega con replay/cancel — Gemini (consola)
- apps/console/src/features/terminal/AdapterInspector.tsx — inspector de permisos, adaptadores y PTY — Gemini (consola)
- apps/console/src/features/terminal/FleetSidebar.tsx — sidebar lateral con leases y chips de capacidad PTY — Gemini (consola)
- apps/console/src/features/terminal/GridContainer.tsx — reja de pestañas de sesiones con repliegue en observación — Gemini (consola)
- apps/console/src/features/terminal/OperatorWorkspace.tsx — workspace con fence de plaza, ciclo de vida PTY y autorización — Gemini (consola)
- apps/console/src/features/terminal/PlazasColgadas.tsx — tira de plazas colgadas con reloj de suelta y `cerrar ahora` — Gemini (consola)
- apps/console/src/features/terminal/PtySessionBar.tsx — barra superior de una sesión PTY abierta (alias/ticket/cuenta atrás) — Gemini (consola)
- apps/console/src/features/terminal/PtySessionDialog.tsx — diálogo de motivo escrito a mano para PTY nueva — Gemini (consola)
- apps/console/src/features/terminal/PtyTerminal.tsx — wrapper React para xterm con reenganche del nodo entre paneles — Gemini (consola)
- apps/console/src/features/terminal/SessionStage.tsx — escenario de una sesión: TUI/PTY/feed, inspección, compositor — Gemini (consola)
- apps/console/src/features/terminal/TerminalPage.test.tsx — render con mocks y avales de PTY — Gemini (consola)
- apps/console/src/features/terminal/TerminalPage.tsx — página del terminal con 6 contadores que se repliegan al abrir sesión — Gemini (consola)
- apps/console/src/features/terminal/TerminalTranscript.tsx — transcript de burbujas in/out con timeline y selección — Gemini (consola)
- apps/console/src/features/terminal/api.test.ts — pruebas del módulo API PTY (timeout, malformed, UNKNOWN inventory, CSRF) — Gemini (consola)
- apps/console/src/features/terminal/api.ts — módulo API PTY: tickets, csrf, inventory, owner takeover — Gemini (consola)
- apps/console/src/features/terminal/cuerpo-del-mensaje.test.ts — pruebas de `textoDelCuerpo` y `previsualizacionRecortada` — Gemini (consola)
- apps/console/src/features/terminal/cuerpo-del-mensaje.ts — constante `CARACTERES_DE_PREVISUALIZACION=240` y lector del body — Gemini (consola)
- apps/console/src/features/terminal/denegaciones.test.tsx — invariantes de los códigos de denegación del gateway — Gemini (consola)
- apps/console/src/features/terminal/denegaciones.ts — traductor de códigos PTY a prosa con `Lo levanta` — Gemini (consola)
- apps/console/src/features/terminal/densidad-observacion.test.tsx — repliegue de los 6 contadores con sesión abierta — Gemini (consola)
- apps/console/src/features/terminal/doctrina.ts — `TEXTO_DOCTRINA` exportado para reusarse en cabecera y footer — Gemini (consola)
- apps/console/src/features/terminal/estilos-en-linea.test.ts — pruebas de no inyección de `<style>` en xterm (CSP) — Gemini (consola)
- apps/console/src/features/terminal/fleet.test.ts — cobertura de la lógica de la flota de terminal — Gemini (consola)
- apps/console/src/features/terminal/fleet.ts — `LiveTuiGate`/`terminalChannelGate`/`fleetTerminalChip`/`leaseStateLabel` — Gemini (consola)
- apps/console/src/features/terminal/live-tui.test.tsx — pruebas de la TUI viva (auto-apertura, controles negativos) — Gemini (consola)
- apps/console/src/features/terminal/nav-availability.test.tsx — pruebas de `terminalNavAvailability` y `configNavAvailability` — Gemini (consola)
- apps/console/src/features/terminal/plazas.test.tsx — pruebas de la trampa del tope de sesiones (cierre, reconciliación, geografía) — Gemini (consola)
- apps/console/src/features/terminal/plazas.ts — `ocupaPlaza`, `plazasOcupadas`, `plazasColgadas`, `minutosParaLiberar` — Gemini (consola)
- apps/console/src/features/terminal/plugin.test.ts — pruebas del gate del plugin y de TUI en vivo — Gemini (consola)
- apps/console/src/features/terminal/plugin.ts — `ultimateTerminalGate`/`terminalChannelGate`/`liveTuiGate` con mismas reglas same-origin — Gemini (consola)
- apps/console/src/features/terminal/pty-connection.ts — ciclo de vida del WebSocket PTY: handshake, resume, control — Gemini (consola)
- apps/console/src/features/terminal/pty-input.ts — cola de input con 8ms de coalesce y `4414` por flood — Gemini (consola)
- apps/console/src/features/terminal/pty-output.ts — escritura de chunks con TextDecoder streaming y worker opcional — Gemini (consola)
- apps/console/src/features/terminal/pty-session.test.ts — pruebas del manager: ticket, control, reconnect, parser DA — Gemini (consola)
- apps/console/src/features/terminal/pty-session.ts — manager externo a React de sesiones PTY: ensurePtySession, attachPtySession — Gemini (consola)
- apps/console/src/features/terminal/pty-socket-stub.ts — doble de WebSocket para tests (cero red) — Gemini (consola)
- apps/console/src/features/terminal/pty-theme.ts — paleta, fuente, `documentoQueNiegaLosEstilos` (override anti-CSP) — Gemini (consola)
- apps/console/src/features/terminal/pty-types.ts — constantes (close codes, MAX_*, `claimReady`), tipos `PtyEntry`/`PtySessionView` — Gemini (consola)
- apps/console/src/features/terminal/redimensionado.test.ts — pruebas de la propagación de `resize` sin SIGWINCH espurios — Gemini (consola)
- apps/console/src/features/terminal/relay-status.test.tsx — pruebas de la clasificación del relay (sin permiso / sin comprobar / no desplegado) — Gemini (consola)
- apps/console/src/features/terminal/relay-status.ts — derivador puro del estado del relay PTY y `useTerminalRelayStatus` — Gemini (consola)
- apps/console/src/features/terminal/session.test.ts — pruebas de `operatorRouteForAgent`, `transcriptForSession`, `formatCountdown` — Gemini (consola)
- apps/console/src/features/terminal/session.ts — ruta de publicación, transcript por sesión, motivo PTY — Gemini (consola)
- apps/console/src/features/terminal/terminal.worker.ts — worker que coalesce chunks PTY a 8 KiB — Gemini (consola)
- apps/console/src/features/terminal/types.ts — `TerminalGrantRequestOutcome` y `RequestTerminalGrant` — Gemini (consola)
- apps/console/src/features/terminal/xterm-csp.css — ⚠ el nombre miente: es el archivo principal de CSS; `@import`s otros tres — Gemini (consola)
- apps/console/src/features/terminal/xterm-csp.test.ts — pruebas estructurales de la hoja CSP de xterm — Gemini (consola)
- apps/console/src/features/topology/AclEdgeList.tsx — lista de aristas ACL con su caption (route/read/control) — Gemini (consola)
- apps/console/src/features/topology/HyperGraph.tsx — hipergrafo SVG con salas (hiperaristas) y aristas ACL — Gemini (consola)
- apps/console/src/features/topology/TenantCards.tsx — tarjetas de tenant con sus salas y miembros — Gemini (consola)
- apps/console/src/features/topology/hypergraph-layout.test.ts — pruebas del layout determinista y no-pisado — Gemini (consola)
- apps/console/src/features/topology/hypergraph-layout.ts — layout del hipergrafo: anclas, relajación, separación, etiquetas — Gemini (consola)
- apps/console/src/features/topology/layout-geometry.ts — geometría: footprint, hash FNV-1a, convexHull, inflateHull, pointInPolygon — Gemini (consola)
- apps/console/src/features/topology/layout-labels.ts — `placeLabels` (etiquetas sin pisarse) y `aclCaption` — Gemini (consola)
- apps/console/src/features/topology/layout-nodes.ts — `collect`, `relax`, `separate`, `anchorEdges`, `arcBetween` — Gemini (consola)
- apps/console/src/lib.test.ts — smoke de `permissionState` y `safe*State` — Gemini (consola)
- apps/console/src/lib.ts — centinelas (`UNKNOWN`, `TODAVIA_NO`, `NO_APLICA`), formateadores y `permissionState` — Gemini (consola)
- apps/console/src/menu-movil.test.ts — pruebas CSS del menú móvil (cuadrícula, `nowrap`) — Gemini (consola)
- apps/console/src/mocks/browser.ts — setupWorker de MSW con ping para mantener vivo el service worker — Gemini (consola)
- apps/console/src/mocks/data.ts — re-exports de los fixtures (topología, actividad, mensajes, colas, cuotas, audit) — Gemini (consola)
- apps/console/src/mocks/fixtures/activity-fixtures.ts — `mockActivity()` con 15 alias, estados variados y aristas de delegación — Gemini (consola)
- apps/console/src/mocks/fixtures/messaging-fixtures.ts — fixtures de status, messages, queues, dlq, adapters, audit — Gemini (consola)
- apps/console/src/mocks/fixtures/quotas-fixtures.ts — `mockQuotas()` con codex/claude/antigravity/opencode — Gemini (consola)
- apps/console/src/mocks/fixtures/topology-config.ts — topología demo: 5 tenants, 15 alias, aristas ACL — Gemini (consola)
- apps/console/src/mocks/handlers.tenant.test.ts — pruebas del mock de perfil/documents tenant-qualified — Gemini (consola)
- apps/console/src/mocks/handlers.ts — handlers MSW para todos los endpoints `/v3/console/*` — Gemini (consola)
- apps/console/src/mocks/server.ts — `setupServer` para tests de Node (vitest) — Gemini (consola)
- apps/console/src/mocks/terminal-demo.test.ts — pruebas de la PTY de mentira (ready fenced, sin trama legacy) — Gemini (consola)
- apps/console/src/mocks/terminal-demo.ts — handlers y WebSocket falso para que la PTY se monte sin backend — Gemini (consola)
- apps/console/src/mocks/terminal-ticket.ts — `mockTerminalGrant`/`mockTerminalTicket` con ticket v1 firmado estructuralmente — Gemini (consola)
- apps/console/src/nav.ts — `NAV_ENTRIES` (8 entradas) y `useNavAvailability` (hook con RBAC + relay) — Gemini (consola)
- apps/console/src/navigation.ts — `navigate`, `redirect`, `onNavClick`, `terminalNavAvailability`, `configNavAvailability` — Gemini (consola)
- apps/console/src/styles.legibilidad-themes.test.ts — pruebas de contraste WCAG 2.1 AA en los dos temas — Gemini (consola)
- apps/console/src/styles.legibilidad.test.ts — pruebas de ancho y de layout móvil — Gemini (consola)
- apps/console/src/styles.tipografia-montada.test.tsx — pruebas del suelo tipográfico sobre vistas montadas — Gemini (consola)
- apps/console/src/styles.tipografia.test.ts — pruebas estructurales de la escala tipográfica — Gemini (consola)
- apps/console/src/vite-env.d.ts — tipos de `import.meta.env.VITE_CAUCE_API_BASE` y `VITE_USE_MOCKS` — Gemini (consola)
- apps/console/src/vocabulario.test.tsx — guardián del vocabulario (sin JSX crudo, sin UNKNOWN, sin ISO cruda) — Gemini (consola)
- apps/console/vite.config.ts — config de Vite: proxy de `/v3`, manual chunks, alias de `@cauce/protocol` — Gemini (consola)
...
- ops/scripts/validate-console-browser-storage.mjs — análisis estático TS: rechaza localStorage/sessionStorage/IndexedDB/caches en apps/console/src — Codex
...
Las suites co-localizadas bajo `src/` (p. ej. `services/gateway/src/*.test.ts`, `apps/console/src/**/*.test.tsx`) ya están descritas en las secciones A–D. Aquí van las suites independientes.
