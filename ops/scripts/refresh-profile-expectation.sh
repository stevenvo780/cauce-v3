...
# The alias own client certificate already carries `read`, which is all this GET needs. Nothing
# here reads, copies or moves credential material: curl opens the files the adapter already uses.
...
# Authenticated as this alias, a route with no tenant can only mean its own, so the script never
# has to learn a tenant mapping that could drift away from the fleet.
route="/v3/console/agents/$alias_name/perfil"
...
try:
    ...
    print("unreadable")
    raise SystemExit(0)
verification = document.get("runtime_verification") or {}
print(
    verification.get("state") or "absent",
    verification.get("generation") or "-",
    "profile" if document.get("exists") else "no-profile",
)
' "$body"
}
...
while :; do
  ...
  if [[ $code == 200 ]]; then
    read -r state generation profile <<<"$(state_of)"
    ...
    log "attempt $attempt: runtime is $state, not yet current"
  else
    log "attempt $attempt: console answered $code"
  fi
...
die "expectation NOT registered for $alias_name after $attempt attempts; last state=$state (runtime not measurable? check cauce-v3-pty@$alias_name)" 1
