...
assert.equal((await readdir(rootless)).filter((name) => /^cauce-v3-container-.*\.service$/u.test(name)).length, 12);
assert.equal((await readdir(path.join(rootless, "configs"))).filter((name) => name.endsWith(".env.example")).length, 12);
...
assert.match(rootlessConfig, /^BUNDLE_CURRENT=\/home\/dev\/\.local\/share\/cauce-v3-adapter\/current$/mu);
