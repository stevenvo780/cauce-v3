...
export function protocolPrompt(
  ...
  return [
    ...
    '- Every notify entry needs "kind": use "decision_request" when you need the owner to decide or authorize, "alert" when something broke and a person must know, "task_complete" when a long task finished, "digest" for a periodic summary. An entry without a valid "kind" is rejected and the whole turn fails.',
