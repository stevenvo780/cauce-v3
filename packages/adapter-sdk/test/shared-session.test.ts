...
class FakeTmux implements TmuxController {
  ...
  async run(args: readonly string[], stdin?: string): Promise<TmuxResult> {
    ...
    if (command === "capture-pane") {
      ...
      if (!this.sessionExists) return { exitCode: 1, stdout: "", stderr: "can't find session" };
      return { exitCode: 0, stdout: this.paneContent, stderr: "" };
    }
...
test("si la reanudacion mata el panel, se rehace EN BLANCO y se dice", async () => {
  ...
  // Antes de rehacerla hay que barrer lo que quedara de la sesión fallida: si tmux la conservara
  // con el panel muerto dentro, el segundo `new-session` chocaría con ella.
  assert.ok(tmux.calls.some((call) => call[0] === "kill-session"));
