...
export const MARCA_PERFIL_INICIO = `<!-- CAUCE:PERFIL v${VERSION_PERFIL} — generado desde la configuración, no editar dentro de este bloque -->`;
 ...
 */
function parDeMarcas(texto, marcaInicio, marcaFin) {
    let inicio = -1;
    for (let busca = texto.indexOf(marcaInicio); busca !== -1; busca = texto.indexOf(marcaInicio, busca + 1)) {
        if (texto.indexOf(marcaFin, busca + marcaInicio.length) !== -1)
            inicio = busca;
    }
    if (inicio === -1)
        return undefined;
 ...
 */
export function bloqueEntreMarcas(texto, marcaInicio, marcaFin) {
 ...
 */
export function conBloqueEntreMarcas(textoOriginal, bloque, marcaInicio, marcaFin) {
