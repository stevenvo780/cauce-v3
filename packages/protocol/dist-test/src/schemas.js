...
export function isAmbiguousAckErrorCode(code) {
 ...
 */
export function clampToRoleBriefLimit(text) {
...
function hasUnsafeAttachmentCodePoint(value) {
 ...
 */
export function messageTimeoutMs(body) {
    if (typeof body !== 'object' || body === null || Array.isArray(body))
        return undefined;
    const parsed = MessageTimeoutMsSchema.safeParse(body.timeout_ms);
...
export const MessageBodySchema = z.record(z.string(), z.unknown()).superRefine((body, context) => {
    ...
    if (body.attachments_v1 === undefined)
        return;
...
const ReservedInternalMessageTypes = new Set(RESERVED_INTERNAL_MESSAGE_TYPES);
const AgentToAgentMessageTypes = new Set(AGENT_TO_AGENT_MESSAGE_TYPES);
/**
 ...
 */
export function isAgentToAgentBody(body) {
    if (typeof body !== 'object' || body === null || Array.isArray(body))
        return false;
    const type = body.type;
    return typeof type === 'string' && AgentToAgentMessageTypes.has(type);
}
const AuthenticatedPublishBodySchema = MessageBodySchema.superRefine((body, context) => {
...
export const NOTIFY_KINDS = ['task_complete', 'decision_request', 'digest', 'alert'];
 ...
 */
export const SUPPORTED_QUOTA_SCHEMA_VERSIONS = [1, 2];
...
}).strict().refine((window) => window.used_percent != null || window.remaining_percent != null || window.used_units != null, { message: 'a window sample needs at least one of used_percent, remaining_percent or used_units' });
 ...
 */
function AgentProfileListBorderSchema(field) {
    return z.array(z.string().refine((text) => countCodePoints(text) <= AGENT_PROFILE_BORDER_TEXT, { message: `a ${field} entry is too long` })).max(AGENT_PROFILE_BORDER_ITEMS, { message: `${field} has too many entries` }).optional();
}
/**
...
export const AgentProfileConfigMutationSchema = z.object({
    ...
    value: z.object({
        purpose: z.string()
            .refine((text) => countCodePoints(text) <= AGENT_PROFILE_BORDER_TEXT, { message: 'purpose is too long' })
            .nullable().optional(),
        role_summary: z.string()
            .refine((text) => countCodePoints(text) <= AGENT_PROFILE_BORDER_TEXT, { message: 'role_summary is too long' })
            .nullable().optional(),
...
export const DeliveryEnvelopeSchema = z.object({
    ...
    self_role: z.string().min(1).superRefine((text, ctx) => {
        ...
        if (codePoints <= ROLE_BRIEF_MAX_CODE_POINTS)
            return;
        ctx.addIssue({
            code: 'custom',
