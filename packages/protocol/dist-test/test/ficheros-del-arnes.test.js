...
import { measureStrictestUnits } from "../src/agent-profile.js";
import { MARCA_FIN, MARCA_INICIO, MARCA_PERFIL_FIN, MARCA_PERFIL_INICIO, } from "../src/marcas-de-bloque.js";
import { ErrorDeTopeDelArnes, FICHEROS_OPENCLAW, TOPES_OPENCLAW, ficherosDelArnes, nombresDelArnes, } from "../src/ficheros-del-arnes.js";
/**
...
function textoDe(ficheros, nombre) {
...
test("openclaw recibe los SIETE ficheros medidos, con esos nombres exactos", () => {
    ...
    assert.deepEqual(ficheros.map((f) => f.nombre), ["SOUL.md", "IDENTITY.md", "USER.md", "MEMORY.md", "HEARTBEAT.md", "AGENTS.md", "TOOLS.md"]);
...
test("cada cara del perfil cae en SU fichero y NO en los demás", () => {
    ...
    for (const [duenno, marcador] of marcadores) {
        for (const fichero of ficheros) {
            if (fichero.nombre === duenno)
                continue;
            assert.doesNotMatch(fichero.texto, marcador, `${marcador} se coló en ${fichero.nombre}; sólo pertenece a ${duenno}`);
        }
    }
});
...
test("MEMORY y HEARTBEAT que YA existen se devuelven intactos byte a byte, y sin escribir", () => {
    ...
    const ficheros = ficherosDelArnes("openclaw", { perfil: perfil(), hechos: hechos() }, new Map([["MEMORY.md", memoriaViva], ["HEARTBEAT.md", latidoVivo]]));
...
test("CONTROL NEGATIVO: un MEMORY con marcas nuestras TAMPOCO se toca", () => {
    ...
    const ficheros = ficherosDelArnes("openclaw", { perfil: perfil(), hechos: hechos() }, new Map([["MEMORY.md", memoriaConMarcas]]));
    const memoria = ficheros.find((f) => f.nombre === "MEMORY.md");
    assert.ok(memoria);
...
test("lo que escribió una persona sobrevive byte a byte, antes y después del bloque", () => {
    ...
    const ficheros = ficherosDelArnes("openclaw", { perfil: perfil(), hechos: hechos() }, new Map([["SOUL.md", `${humano}${cola}`]]));
...
test("regenerar sobre lo ya generado NO duplica el bloque", () => {
    ...
    const segunda = textoDe(ficherosDelArnes("openclaw", contexto, new Map([["SOUL.md", primera]])), "SOUL.md");
...
test("el bloque SELLADO del contrato no se toca: el generador sólo escribe el suyo", () => {
    ...
    const ficheros = ficherosDelArnes("openclaw", { perfil: perfil(), hechos: hechos() }, new Map([["AGENTS.md", `${sellado}\n\nde una persona\n`]]));
    const agents = textoDe(ficheros, "AGENTS.md");
...
test("el ORDEN EN QUE SE CONSTRUYÓ el perfil no cambia un solo byte", () => {
    ...
    assert.deepEqual(ficherosDelArnes("openclaw", { perfil: derecho, hechos: hechos() }).map((f) => f.texto), ficherosDelArnes("openclaw", { perfil: alReves, hechos: hechos() }).map((f) => f.texto));
});
...
test("un fichero que pasa de 60.000 da error CLARO, y no un fichero truncado", () => {
    ...
    assert.throws(() => ficherosDelArnes("openclaw", {
...
test("el tope se mide en la unidad de openclaw (UTF-16), no en bytes ni en puntos de código", () => {
    ...
    assert.throws(() => ficherosDelArnes("openclaw", { perfil: perfil({ purpose: astral }), hechos: hechos() }), ErrorDeTopeDelArnes);
});
test("claude y codex no tienen tope declarado: un perfil enorme NO se rechaza por el de openclaw", () => {
    ...
    const enorme = "x".repeat(TOPES_OPENCLAW.porFichero + 1);
    assert.doesNotThrow(() => ficherosDelArnes("claude", { perfil: perfil({ purpose: enorme }), hechos: hechos() }));
});
// ── EL PERFIL VACÍO ──────────────────────────────────────────────────────────────────────────
...
//# sourceMappingURL=ficheros-del-arnes.test.js.map