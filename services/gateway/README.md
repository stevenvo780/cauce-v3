...
**Aviso:** `src/app.ts` ya está modularizado (283 líneas que sólo componen `routes/*`); la partición de `terminal/plugin.ts` vive en `services/gateway/src/terminal/{session-control,relay-proxy,authority,registry,governance-probes,audit,tickets,types,config}.ts`. publish-intents y chain-gates están SIEMPRE montadas (`routes/console-publish.ts` y `routes/chain-gates.ts`): ya no hay interruptor que las apague. `GatewayRepository` (`src/app.ts`) se deriva de `CauceRepository` con `Pick` y no tiene métodos opcionales; las firmas que el gateway estrecha a propósito se declaran a mano.
