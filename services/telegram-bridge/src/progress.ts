...
export class TelegramBridgeProgress implements TelegramLoopObserver {
  ...
  renderMetrics(): string {
    ...
    return [
      ...
      `cauce_telegram_process_start_time_seconds ${this.startedAt / 1_000}`,
      '# HELP cauce_telegram_pollers Number of configured poller loops.',
      '# TYPE cauce_telegram_pollers gauge',
      `cauce_telegram_pollers ${pollers.length}`,
      ...
      `cauce_telegram_pollers_stale ${state.stale_pollers}`,
      ...
      `cauce_telegram_poll_ticks_total{result="success"} ${pollSuccesses}`,
      `cauce_telegram_poll_ticks_total{result="error"} ${pollFailures}`,
      `cauce_telegram_poll_ticks_total{result="fenced"} ${pollers.reduce((total, loop) => total + loop.fenced, 0)}`,
      ...
      `cauce_telegram_oldest_poll_tick_timestamp_seconds ${oldestPollTick}`,
      ...
      `cauce_telegram_egress_ticks_total{result="success"} ${this.egress?.successes ?? 0}`,
      `cauce_telegram_egress_ticks_total{result="error"} ${this.egress?.failures ?? 0}`,
      `cauce_telegram_egress_ticks_total{result="fenced"} ${this.egress?.fenced ?? 0}`,
      ...
      `cauce_telegram_egress_last_tick_timestamp_seconds ${(this.egress?.tickAt ?? 0) / 1_000}`,
      ...
      `cauce_telegram_health_observed_timestamp_seconds ${now / 1_000}`,
      ''
    ].join('\n');
  }
}
